...
class Singleton {
   ...
   shared_ptr<OdomChassisController> chassisRev;
   shared_ptr<ChassisController> chassisOld;
   ...
   Singleton() {
     chassis =
       ...
       chassisOld = //Open loop version
            ChassisControllerBuilder()
              .withMotors({-20,14,-12},{2,-6,10}) // first set is left, 2nd set is right (- means reversed)
              .withDimensions(AbstractMotor::gearset::green, {{3.30_in, 7.20_in, 4.3_in, 2.80_in}, 1024})
              .build(); // build an odometry chassis

      chassisOld->getModel()->setMaxVelocity(20);
   }
   ...
   shared_ptr<OdomChassisController> getChassis() {
    ...
    shared_ptr<OdomChassisController> getChassisRev() {
      return this -> chassisRev;
     }

    shared_ptr<ChassisController> getChassisOld() {
      return this -> chassisOld;
     }
};
